...
public class HUDController : CanvasLayer
{
    ...
    private float GetHighestBodyValue()
    {
        var valuesList = new List<float>();

        valuesList.Add((float)headProgress.Value);
        valuesList.Add((float)torsoProgress.Value);
        valuesList.Add((float)armLProgress.Value);
        valuesList.Add((float)armRProgress.Value);
        valuesList.Add((float)legsProgress.Value);
